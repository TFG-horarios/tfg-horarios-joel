...
import { ClassroomReservationNotificationAdapter } from './classroom-reservation-notification.adapter';
import type { CreateNotificationUseCase } from '@/modules/notification/application/create-notification.usecase';

describe('ClassroomReservationNotificationAdapter', () => {
  const executeMock = mock();
  const createNotificationUseCaseMock = {
    execute: executeMock,
  } as unknown as CreateNotificationUseCase;

  const adapter = new ClassroomReservationNotificationAdapter(
