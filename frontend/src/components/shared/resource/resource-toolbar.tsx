...
function getFilterElements(node: ReactNode): React.ReactElement[] {
  ...
  React.Children.forEach(node, (child) => {
    if (React.isValidElement(child)) {
      const element = child as React.ReactElement<{ children?: React.ReactNode; paramKey?: string }>;
...
export function ResourceToolbar({
  ...
  const primaryFilters = isThresholdExceeded ? actualFilters.slice(0, 2) : actualFilters;
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3 w-full pb-4 border-b border-border/50">
      <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
        {search && (
          <div className="w-full sm:w-72 flex-none">{search}</div>
        )}
                ...
                <Popover open={openDesktop} onOpenChange={setOpenDesktop}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      <span>Filtros</span>
                      {hasActiveSecondary && (
                        <span className="font-semibold">({activeSecondaryCount})</span>
                      )}
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex flex-col gap-3">
                        {secondaryFilters.map((el, idx) => (
                          <div key={idx} className="w-full [&_button]:!w-full [&_button]:lg:!w-full [&_input]:!w-full [&_input]:lg:!w-full [&_select]:!w-full [&_.select-trigger]:!w-full [&_div]:!w-full">
                            {el}
                          </div>
                        ))}
                      </div>
