...
import {
  fetchPaginatedItineraries,
  fetchAllItineraries,
  fetchItineraryIdentifiers,
} from './queries';
...
export async function fetchPaginatedItinerariesAction(
  ...
  return fetchPaginatedItineraries(organizationId, { ...query, page });
}
...
export async function fetchItineraryIdentifiersAction(
  organizationId: string
): Promise<ItineraryIdentifierDTO[]> {
  try {
    return await fetchItineraryIdentifiers(organizationId);
  } catch (error) {
    console.error(
